...
const getFileIcon = (filePath) => {
  ...
  if (["pdf"].includes(extension)) return <ImFilePdf className={css.iconpdf} />;
  if (["xls", "xlsx"].includes(extension)) return <ImFileExcel className={css.iconxls} />;
  if (["doc", "docx", "rtf"].includes(extension)) return <ImFileWord className={css.icondoc} />;
  ...
  return (
    <div className={css.wrapper}>
        ...
        <BsDownload
          className={css.icon}
          onClick={() =>
            handleDownload(fileUrl, shortName)
          }
        />
      </div>
          );
