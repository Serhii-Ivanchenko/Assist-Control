...
const period = [{ id: 1, name: "Кожного разу" },
...
const data = [
  { recId: 1, parent: 0, name: "Автосервіс", action: 4, acc: 2, period: 2, activity: true },
  { recId: 2, parent: 1, name: "Співробітників", activity: true },
  { recId: 3, parent: 2, name: "Керівник", action: 4, acc: 2, period: 1, activity: true },
  { recId: 4, parent: 2, name: "Менеджер", action: 4, acc: 2, period: 1, activity: true },
  { recId: 5, parent: 2, name: "Адміністратор", action: 4, acc: 2, period: 1, activity: true },
  { recId: 6, parent: 2, name: "Механік", action: 4, acc: 2, period: 1, activity: true },
  { recId: 7, parent: 0, name: "Кліента", action: 4, acc: 2, period: 1, activity: true },
  { recId: 8, parent: 0, name: "Постачальникі", action: 4, acc: 4, period: 1, activity: true },
  { recId: 9, parent: 0, name: "Запчастин", action: 4, acc: 4, period: 1, activity: true },
];
...
const RatingPart = () => {
  ...
  const handleSelectChange = (recId, field, value) => {
    setEditedData((prev) =>
      prev.map((row) => (row.recId === recId ? { ...row, [field]: value } : row))
    );
  };
      ...
      {editedData.map((item) => {
        ...
        return (
          <React.Fragment key={item.recId}>
            ...
            {item.parent === 0 && (
              ...
              >
                ...
                <div style={{ flex: "0 0 3%" }}>
                  ...
                  style={{  fill: item.activity ? "var(--green-btn-normal)" : "var(--red)", }}
                   /> 
                </div>
              </div>
            )}
            ...
            {isAccordion && (
              <div
                className={css.flexitem} style={{cursor: "pointer"}}
                onClick={() => toggleAccordion(item.recId)}
              >
                <div style={{ flex: "0 0 20%" }} className={css.dictionitemflex}>{item.name}
                    <BsFillCaretDownFill
                    className={`${css.iconbtn} ${
            ...
            {isChild && openAccordions[item.parent] && (
              ...
              >
                <div style={{ flex: "0 0 20%" , marginLeft: "20px"}} >{item.name}</div>
